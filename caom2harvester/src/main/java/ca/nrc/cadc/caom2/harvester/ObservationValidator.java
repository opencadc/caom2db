...
public class ObservationValidator extends Harvester {
        ...
        try {
            ...
            if (!this.service) {
                tmpSrcState = srcObservationDAO.getObservationList(collection, startDate, end,
                        batchSize + 1);
                tmpSrc = srcObservationDAO.getList(Observation.class, startDate, end,
                        batchSize + 1);
            } else {
                tmpSrcState = srcObservationService.getObservationList(collection, startDate, end,
                        batchSize + 1);

                List<WorkerResponse> aux = srcObservationService.getList(collection, startDate, end,
                        batchSize + 1);
            ...
            List<ObservationError> errlist = calculateErroneousObservations(tmpSrcState,
                    tmpDstState, tmpSrc, tmpDst);
            ...
            entityListSrcComplete = wrapObservation(tmpSrc);
            if (entityListSrcComplete.size() > 0) {
                curLastModified = entityListSrcComplete.get(entityListSrcComplete.size() - 1).entity
                        .getMaxLastModified();
            }

            ret.found = entityListSrcComplete.size();
            log.info("found: " + entityListSrcComplete.size());
                ...
                try {
                        ...
                        if (o != null) {
                            ...
                            try {
                                ...
                                HarvestSkipURI skip = harvestSkip.get(source, cname,
                                        o.getObs().getURI().getURI());
                                if (skip == null) {
                                    skip = new HarvestSkipURI(source, cname,
                                            o.getObs().getURI().getURI(), skipMsg);
                                } else {
                    ...
                    if (oops instanceof Error) {
                        ...
                        if (str.contains("spherepoly_from_array")) {
                            log.error(
                                    "UNDETECTED illegal polygon: " + o.getObs().getURI().getURI());
                        } else
    ...
    private List<ObservationError> calculateErroneousObservations(
            List<ObservationState> tmpSrcState, List<ObservationState> tmpDstState,
            List<Observation> tmpSrc, List<Observation> tmpDst) {
        ...
        for (int i = 0; i < tmpSrcState.size(); i++) {
            ...
            if (!osSrc.getURI().getObservationID().equals(oSrc.getURI().getObservationID())) {
                ...
                ObservationError oe = new ObservationError(oSrc,
                        "Observation " + oSrc.getObservationID()
                ...
                if (!(osSrc.accMetaChecksum != null && oSrc.getAccMetaChecksum() != null
                    ...
                    ObservationError oe = new ObservationError(oSrc, "Observation "
                            + oSrc.getObservationID()

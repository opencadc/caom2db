...
public class ArtifactHarvester implements PrivilegedExceptionAction<Integer> {
	...
	public ArtifactHarvester(ArtifactDAO artifactDAO, String[] dbInfo, ArtifactStore artifactStore, String collection,
	...
	public Integer run() throws Exception {
		...
		try {
				...
				if (!acceptedFilePrefixes.contains(firstChar)) {
					...
					try {
						...
						if ("mast".equalsIgnoreCase(artifact.getURI().getScheme())) {
							...
							log.debug("Artifact " + artifact.getURI() + " with MD5 " + artifact.contentChecksum
									+ " exists: " + exists);
							if (!exists) {
			...
			log.info("Discovered " + downloadCount + " total artifacts eligible for download. (" + newDownloadCount
					+ " new)");
		}

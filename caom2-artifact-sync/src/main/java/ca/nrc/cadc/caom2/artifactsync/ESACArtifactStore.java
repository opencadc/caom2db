...
public class ESACArtifactStore implements ArtifactStore
{
    ...
    {
        init();
        ESACDataURI dataURI = new ESACDataURI(artifactURI.toString());
        File file = createFile(dataURI);
        return file.exists();
    }
    ...
    public void store(URI artifactURI, URI checksum, InputStream data) throws TransientException
    {
        ...
        {
            throw new TransientException("Unable to create file: " + file);
        }
    }
    ...
    private File createFile(ESACDataURI dataURI)
    {
        return new File(dataURI.getArchivePath());
    }
